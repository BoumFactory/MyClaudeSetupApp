...
Pour garantir la cohérence avec le package bfcours :

- **Citations** : Utiliser `\frquote{texte}` pour toutes les citations
- **Mise en valeur** : Utiliser `\textbf{texte}` pour les termes importants
- **Vocabulaire** : Privilégier les environnements de définition ou blocs
- **Nombres** : Toujours utiliser `\num{valeur}` avec siunitx pour la cohérence

Exemple :
```latex
\begin{frame}{Exemple de citation}
  Selon le théorème, \frquote{la somme des angles d'un triangle vaut \num{180} degrés}.
\end{frame}
```

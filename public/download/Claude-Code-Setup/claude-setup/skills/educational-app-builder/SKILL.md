...
### Ne JAMAIS lancer l'application toi-même

**IMPORTANT** : Claude NE DOIT JAMAIS lancer l'application lui-même (npm start, pnpm dev, python app.py, etc.) car cela :
- Consomme des ressources système inutilement
- Bloque des processus en arrière-plan
- Peut créer des conflits de ports si l'utilisateur a déjà l'app lancée
- Génère des logs superflus dans la conversation

**Workflow correct** :
1. Claude crée/modifie les fichiers de l'application
2. Claude indique à l'utilisateur comment lancer l'application
3. **L'utilisateur lance l'application lui-même**
4. Claude interagit avec l'application via :
   - Les serveurs MCP disponibles
   - La modification des fichiers sources
   - L'analyse des logs fournis par l'utilisateur

### Interaction avec les applications existantes

Pour les applications déjà créées (comme `cahier-de-textes`) :
- Modifier les fichiers React/Electron via les outils Read/Write/Edit
- Utiliser les serveurs MCP si disponibles pour interagir avec la base de données
- Attendre que l'utilisateur fournisse les retours sur le comportement de l'app
- Ne jamais relancer l'application après modification - l'utilisateur gère le rechargement

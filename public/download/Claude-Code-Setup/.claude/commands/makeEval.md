...
- `--qcm <nombre_questions>` : Nombre de questions du QCM (optionnel, défaut: 12)
  - Mettre `0` ou `false` pour désactiver le QCM
  - Standard sujet 0 : 12 questions @ 0,5 pt = 6 points
...
```bash
# Évaluation sur une séquence complète (avec QCM 12 questions par défaut)
/makeEval "1. Cours\2nde\Sequence-Vecteurs" --duree 50

# Évaluation sans QCM
/makeEval "1. Cours\3eme\Sequence-Pythagore" --duree 60 --qcm 0

# QCM avec 8 questions seulement (4 points)
...
# QCM 10 questions avec exclusions
/makeEval "1. Cours\1ere_ens_scient\Sequence-Pourcentages" --duree 50 --qcm 10 --demande "exclure les tableaux de contingence du QCM"
```
...
```
┌─────────────────────────────────────────────────┐
│ Exercice 1 : QCM (X points) - PAR DÉFAUT       │
│ - N questions @ 0,5 point chacune              │
│ - Par défaut N=12 (6 points, format sujet 0)  │
...
- **qcm-builder** : Génère automatiquement un QCM adapté au niveau (si activé, par défaut 12 questions)
